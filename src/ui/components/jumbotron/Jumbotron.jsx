...
function Jumbotron() {
  const [images, setImages] = useState(() => {
    const first = jumboIMG[Math.floor(Math.random() * jumboIMG.length)];
    const second = getRandomImg(first);
    return [first, second];
  });
  const [activeIndex, setActiveIndex] = useState(0);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const nextImage = getRandomImg(images[activeIndex]);
      ...
      img.onload = () => {
        setImages(prev => {
          ...
          newImages[inactiveIndex] = nextImage;
          return newImages;
        });
  ...
  const handleMouseEnter = () => {
    setIsHovered(true);
    setTimeout(() => setSymbolSrc(IMG.nouma_onlySymbol_green), 100);
  };

  const handleMouseLeave = () => {
    setIsHovered(false);
    setTimeout(() => setSymbolSrc(IMG.nouma_onlySymbol_white), 100);
  };
  ...
  return (
    <section id="jumboCon" className="container-fluid">
      ...
      <div id="jumboFrame" className="row">
        <div
          id="jumboInnerFrame"
          className={`col ${isHovered || isMenuOpen ? "hovered-border" : ""}`}
        ></div>
        ...
        <div id="jumboInnerLogo" className="col">
          ...
          <div id="nameRow" className="row">
            <img
              id="noumaNameLine"
              className="col-auto"
              src={IMG.nouma_lineName_white}
              alt="nouma name"
            />
            <img
              id="noumaNameWrap"
              className="col-auto"
              src={IMG.nouma_onlyNameRight_white}
              alt="nouma name"
            />
          </div>
        </div>
